...
def process_pair(pair_dict: dict, alligner: BaseStructureAlligner, ligand: str, result_list: ListProxy, min_ligand_atoms: int = 3) -> None:
    if len(pair_dict['ref_chain']) != 1 or len(pair_dict['mov_chain']) != 1:
        return result_list.append((ligand, pair_dict["ref_name"], pair_dict["mov_name"], pair_dict["ref_chain"], pair_dict["mov_chain"], None, None, None,  None, None, pair_dict['cath_level'] ,None, None, "invalid chain given"))
    ...
    rotations, translations, rmse, coverage, message = pair.find_transformations(alligner)
