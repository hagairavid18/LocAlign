...
class VirtualSoftBB(SoftBBBase):
    def __init__(self, loss: dict[str, Any], optimizer: dict[str, Any], input_layer, virtual_layer: dict, scalar_layer: dict | None = None, max_iter: int = 5, compute_pocket_importance: bool = False):
    ...
    def _create_virtual_coordinates(self, embedding: torch.Tensor, frames: torch.Tensor, mask: torch.Tensor, mode: str) -> torch.Tensor:
        """
    ...
    def _compute_soft_bb_algorithm(self, batch: dict[torch.Tensor]) -> tuple[dict[str, torch.Tensor], dict[str, torch.Tensor]]:
        ...
        virtual_src_coord, virtual_tar_coord = self._create_virtual_coordinates(src_embedding, batch['src_frames'], batch['src_mask'], 'src'), self._create_virtual_coordinates(tar_embedding, batch['tar_frames'], batch['tar_mask'], 'tar')
