...
class VirtualSoftBB(SoftBBBase):
    def __init__(self, loss: dict[str, Any], optimizer: dict[str, Any], input_layer, virtual_layer: dict, max_iter: int = 5, n_iter_train: int = 2, plot_dir: str | None = None) -> None:
    ...
    def _create_virtual_coordinates(self, embedding: torch.Tensor, frames: torch.Tensor, mask: torch.Tensor, metadata: str) -> torch.Tensor:
        ...
        offsets = torch.clamp(offsets, min=-5.0, max=5.0)
    ...
    def _compute_soft_bb_algorithm(self, batch: dict[torch.Tensor]) -> tuple[dict[str, torch.Tensor], dict[str, torch.Tensor]]:
        ...
        virtual_src_coord, virtual_tar_coord = self._create_virtual_coordinates(src_embedding, batch['src_frames'], batch['src_mask'], metadata=batch['metadata']), self._create_virtual_coordinates(tar_embedding, batch['tar_frames'], batch['tar_mask'], metadata=batch['metadata'])
        ...
        return optimal_transformation, mask_2d
    ...
    def training_step(self, batch: dict[torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        transformation_dict, _ = self._compute_soft_bb_algorithm(batch)
    ...
    def validation_step(self, batch: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        """        
        batch = move_batch_to_device(batch, self.device)
        transformation_dict, mask = self._compute_soft_bb_algorithm(batch)
