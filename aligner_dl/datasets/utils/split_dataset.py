        ...
        for row_idx, row in df.iterrows():
            for prot_col, chain_col in [('ref_protein', 'ref_chain'), ('mov_protein', 'mov_chain')]:
                ...
                if key not in unique_triples:
                    pdb_file = os.path.join(base_data_path, ligand_id, f"{protein_id}_non_ligand.ent")
                    try:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--mmseq_id_threshold', type=float, default=0.6, help="MMseqs2 sequence identity threshold for clustering")


    args = parser.parse_args()

    # Call the function to split the CSV
    split_csv(args.input_csv, args.output_dir, test_size=args.test_size, val_size=args.val_size, base_data_path=LIGAND_DIR, mmseq_id_threshold=args.mmseq_id_threshold)
