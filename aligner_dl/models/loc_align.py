...
class LocAlign(SoftBBBase):
    ...
    def inference_step(self, batch: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        curr_loss, loss_dict = self._compute_loss(batch, all_iter_results[-1]['pred_R'], all_iter_results[-1]['pred_t'], all_iter_results[-1]['corr_rmsd'], all_iter_results[-1]['embedding_similarity'], all_iter_results[-1]['gap'], inference=True)
