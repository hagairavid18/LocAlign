...
class RecyclingModule(nn.Module):
    ...
    def __init__(self,
                 ...
                 recycle_scalar: bool = True,
                 recycle_graph: bool = True,
        ...
        self.fourier_vectors = torch.stack(torch.meshgrid(fourier_grid_1d,fourier_grid_1d,fourier_grid_1d,indexing='ij'),axis=-1).reshape([-1,3]).transpose(0,1)[:,1:]
        fourier_norms = torch.sqrt(torch.sum(self.fourier_vectors**2,axis=0))
        self.fourier_vector_norms = torch.cat( (fourier_norms,fourier_norms))
        
        self.rbf_scales = nn.Parameter(  ( fourier_min_length / (2*torch.pi) * torch.linspace(0, 4 , fourier_n_rbf_functions) )**2 )        
