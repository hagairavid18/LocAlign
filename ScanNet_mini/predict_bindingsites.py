...
ubiquitin_model_name_MSA = 'ScanNet_ubiquitin'
ubiquitin_model_MSA = ['ScanNet_ubiquitin_autoregressive_config9_31_08_%s'%index for index in range(1,6)]

ubiquitin_model_name_noMSA = 'ScanNet_ubiquitin_noMSA'
ubiquitin_model_noMSA = ['ScanNet_ubiquitin_autoregressive_config9_noMSA_30_08_%s'%index for index in range(1,6)]

interface_model_folder = model_folder
epitope_model_folder = model_folder
idp_model_folder = model_folder
ubiquitin_model_folder = model_folder
...
def predict_interface_residues(
    ...
    # Parse pdb files.
        ...
        if not permissive:
            try:
                # pdbparser = PDBio.Bio.PDB.PDBParser()
                # mmcifparser = PDBio.Bio.PDB.MMCIFParser()
                # _load_chains = partial(PDBio.load_chains,pdbparser=pdbparser,mmcifparser=mmcifparser)
                _load_chains = PDBio.load_chains
                query_chain_objs = [x[1] for x in pool.starmap(_load_chains,zip([None for _ in query_chain_ids],
                    query_chain_ids,pdb_file_locations))]   
                for i in range(npdbs):
                    if query_chain_ids[i] == 'all':
                        query_chain_ids[i] = [(chain_obj.get_full_id()[1], chain_obj.get_full_id()[2])
                                                 for chain_obj in query_chain_objs[i]]
                    elif query_chain_ids[i] == 'upper':
                        query_chain_ids[i] = [(chain_obj.get_full_id()[1], chain_obj.get_full_id()[2])
                                                 for chain_obj in query_chain_objs[i] if (chain_obj.get_full_id()[2].isupper() | (chain_obj.get_full_id()[2] == ' ') )]
                    elif query_chain_ids[i] == 'lower':
                        query_chain_ids[i] = [(chain_obj.get_full_id()[1], chain_obj.get_full_id()[2])
                                                 for chain_obj in query_chain_objs[i] if chain_obj.get_full_id()[2].islower()]
                query_chain_names = [[query_names[i] + '_%s_%s' %
                                      query_chain_id for query_chain_id in query_chain_ids[i]] for i in range(npdbs)]
            except Exception as e:
                print('Failed to parse at least one example',e)
        else:
            i = 0
            while i < npdbs:
                try:
                    ...
                    if not (os.path.exists(target_location) & (~overwrite_MSA)):
                        if sequence in query_sequences[i][:j]:
                          ...
                          target_location = MSA_folder + 'MSA_' + \
                            ...
                            print(f'i={i},{query_chain_names[i][j]}, no MSA found. Building it using {pipeline_MSA.homology_search}', file=logfile)
                            homology_search = sequence_utils.call_hhblits if pipeline_MSA.homology_search == 'hhblits' else sequence_utils.call_mmseqs
                            homology_search(sequence, target_location,cores=cores)
    ...
    if layer is not None:
        assert multi_models == False,'Intermediate output extraction not supported with multi-models'
        if isinstance(layer,list):
            layer_outputs = []
            for layer_ in layer:
                if layer_ is None:
                    layer_outputs.append(model_obj.model.get_layer('classifier_output').output)
                elif layer_ == 'nearest_neighbor_search_atom':
                    layer_outputs.append(model_obj.model.get_layer('nearest_neighbor_search').output[0])
                elif layer_ == 'nearest_neighbor_search_aa':
                    layer_outputs.append(model_obj.model.get_layer('nearest_neighbor_search').output[1])                    
                elif layer_ == 'nearest_neighbor_search_graph':
                    layer_outputs.append(model_obj.model.get_layer('nearest_neighbor_search').output[2])
            ...
            model_obj.Lmax_output = int(model_obj.model.get_layer(layer).output.shape[1])
        return_all = True
    elif 'ubiquitin' in model_name:
        return_all = True
    else:
        return_all = False
    ...
    if assembly:
        ...
        if multi_models:
            if aggregate_models:
                 ...
                 query_predictions = wrappers.wrap_list([prediction/len(model_objs) for prediction in query_predictions])
            else:
    ...
    for chain_obj, sequence, MSA_file_location,PWM in zip(query_chain_objs[i], query_sequences[i], query_MSAs[i],query_PWMs[i])], padded=padded)
            if multi_models:
                if aggregate_models:
                     ...
                     predictions = wrappers.wrap_list([prediction/len(model_objs) for prediction in predictions])
                else:
    ...
    if ('ubiquitin' in model_name) & ('all' not in model_name) & (layer is None):
        query_predictions = [ predictions[...,1] * predictions[...,-1] for predictions in query_predictions]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mode', dest='mode',
                   default='interface',
                   help='Prediction mode (interface, epitope, idp, ubiquitin)')
    ...
    if '.txt' in input:
        with open(input,'r') as f:
            for line in f:
                pdb,chain_ids = PDBio.parse_str(line.replace('\n',''))
    ...
    elif args.mode == 'ubiquitin':
        model_folder = ubiquitin_model_folder
        chimera_thresholds = [0.1, 0.4]
        if args.use_MSA:
            model_name = ubiquitin_model_name_MSA
            model = ubiquitin_model_MSA
        else:
            model_name = ubiquitin_model_name_noMSA
            model = ubiquitin_model_noMSA

    elif args.mode[:-1] == 'ubiquitin': # ubiquitin1, ubiquitin2, ubiquitin3, ubiquitin4, ubiquitin5
        fold = int(args.mode[-1]) - 1
        model_folder = ubiquitin_model_folder
        chimera_thresholds = [0.1, 0.4]
        if args.use_MSA:
            model_name = ubiquitin_model_name_MSA + str(args.mode[-1])
            model = ubiquitin_model_MSA[fold]
        else:
            model_name = ubiquitin_model_name_noMSA  + str(args.mode[-1])
            model = ubiquitin_model_noMSA[fold]
    else:
    ...
    predict_interface_residues(
        ...
        overwrite_MSA=False,
        permissive=False,#args.permissive,
