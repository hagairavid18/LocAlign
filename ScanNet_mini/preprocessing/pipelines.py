    ...
    def build_and_process_dataset(self,
        ...
        if ncores>1:
            ...
            for ksuccess in range(ncores):
                if len(batch_outputs[ksuccess]) > 0:
                    input_is_list = isinstance(batch_outputs[ksuccess][0],list)
                    ninputs = len(batch_outputs[ksuccess][0]) if input_is_list else 1
                    if has_labels:
                        output_is_list = isinstance(batch_outputs[ksuccess][1],list)
                        noutputs = len(batch_outputs[ksuccess][1]) if output_is_list else 1
                    break

            if input_is_list:
                inputs = [[] for _ in range(ninputs)]
                for batch_output in batch_outputs:
                    if len(batch_output[0])>0:
            ...
            for b, origin in enumerate(list_origins):
                ...
                try:
                    ...
                    if ('PWM' in self.requirements) | ('conservation' in self.requirements):
                        ...
                        if self.homology_search == 'hhblits':
                            homology_search = sequence_utils.call_hhblits
                        elif self.homology_search == 'mmseqs':
                            homology_search = sequence_utils.call_mmseqs                            
                        else:
                            raise ValueError
                        
                        MSA_files = [homology_search(sequence, output_alignment,overwrite=overwrite) for sequence, output_alignment in
...
class ScanNetPipeline(Pipeline):
    ...
    def __init__(self,
        ...
        assert(homology_search in ['hhblits','mmseqs'])
        self.homology_search = homology_search
    ...
    def process_example(self,
        ...
        # print('here',chain_obj,sequence,MSA_file,atomic_coordinates,atom_ids,PWM,labels)
        ...
        if self.with_atom:
            ...
            if self.atom_features == 'type':
                ...
                natom_features = 38
            atom_attributes = atom_attributes[:,np.newaxis]
        ...
        if labels is not None:
            if (self.nclasses>0) & (labels.dtype in [bool,int]):
                outputs = binarize_categorical(labels, self.nclasses).astype(curr_int)
            else:
                outputs = labels
            if self.padded:
