...
def get_aa_frameCloud(atom_coordinates, atom_ids, verbose=True, method='triplet_backbone'):
    ...
    aa_to_atom_indices = _get_aa_to_atom(atom_coordinates)
    aa_clouds = np.array(aa_clouds)
    aa_triplets = np.array(aa_triplets, dtype=np.int32)
    return aa_clouds, aa_triplets, aa_to_atom_indices
    # return aa_clouds, aa_triplets, aa_indices


def _get_aa_to_atom(atom_coordinates,N_atom_per_aa_max = 14):
    N_aa = len(atom_coordinates)
    aa_to_atom_indices = np.repeat(np.arange(N_atom_per_aa_max)[np.newaxis] ,N_aa,axis=0).astype(np.int32)
    atom_count = 0
    for k in range(N_aa):
        natoms_in_aa =  len(atom_coordinates[k])
        aa_to_atom_indices[k,:natoms_in_aa] += atom_count
        aa_to_atom_indices[k,natoms_in_aa:] = -1
        atom_count += natoms_in_aa
    return aa_to_atom_indices
...
def _add_virtual_atoms(atom_clouds, atom_triplets, verbose=True):
    ...
    for n in range(natoms):
        ...
        if case1:  # Atom has at least two covalent bonds.
            ...
            if verbose:
                print('Pathological case, atom has no bonds at all', triplet[0], atom_clouds[triplet[0]])
