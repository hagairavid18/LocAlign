...
class ProteinPair:
    ...
    def compute_rmsd_torch(coordinates: torch.Tensor, gt_R: torch.Tensor, gt_t: torch.Tensor, pred_R: torch.Tensor, pred_t:torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        transformed_points_1 = torch.matmul(coordinates, gt_R.transpose(1, 2)) + gt_t[:,:3].unsqueeze(1)
        
        transformed_points_2 = torch.matmul(coordinates, pred_R.transpose(1, 2)) + pred_t.unsqueeze(1)
